import { mod } from 'micro-math'
import type { HSLA, RGB, RGBA } from '../../types'
...
export const convertRGBToHSL: ConvertRGBToHSL = (
  ...
  hue = mod(Math.round(hue), 0, 360, 'max')

  const lightness = Math.round(cMean * 100)

  const saturation = Math.round((cDelta === 0 ? 0 : cDelta / (1 - Math.abs(2 * cMean - 1))) * 100)
